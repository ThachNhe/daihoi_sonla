...
signInAnonymously(auth)
   .then(() => console.log("✅ Anonymous login thành công"))
...
async function cleanupOldImages() {
   try {
      const oneMinuteAgo = Date.now() - 15 * 60 * 1000; // 1 phút trước
      const storageRef = ref(storage, 'images');
      const listResult = await listAll(storageRef);
      console.log(listResult)

      // Dùng map để chạy song song, tránh chờ từng cái
      const deletePromises = listResult.items.map(async (itemRef) => {
         try {
            ...
            if (createdAt < oneMinuteAgo) {
               ...
               console.log(`🗑️ Xóa ảnh: ${itemRef.name}`);
            }
         } catch (err) {
            console.warn(`⚠️ Không thể xử lý ${itemRef.name}:`, err);
         }
      });
   ...
   try {
      ...
      domtoimage.toPng(node, options)
         ...
         .then(async (data1) => {
            ...
            // 📋 Nút sao chép link ảnh
            const copyBtn = document.createElement("button");
            copyBtn.textContent = "📋 Sao chép đường dẫn ảnh";
            ...
            copyBtn.addEventListener("click", async () => {
               try {
                  ...
                  setTimeout(() => (copyBtn.textContent = "📋 Sao chép đường dẫn ảnh"), 2000);
               } catch (err) {
                  console.log(err);
                  alert("Không thể sao chép, vui lòng tự copy đường dẫn ảnh.");
               }
            });
  ...
  <div style="
    ...
    <div style="
      ...
      <p style="font-size: 14px; color: #444; margin-bottom: 10px; line-height: 1.5;">
        Vui lòng chờ ảnh hiển thị hoàn tất, sau đó:
      </p>

      <ul style="text-align: left; color: #555; font-size: 13px; line-height: 1.5; margin: 0 0 12px 20px; padding: 0;">
        <li>📷 Nhấn và giữ vào ảnh, chọn <b>Tải ảnh</b> hoặc <b>Lưu ảnh xuống</b> (tuỳ thiết bị).</li>
        <li>🔗 Hoặc sao chép đường dẫn ảnh để sử dụng sau này. (Đường dẫn chỉ có hiệu lực trong 15 phút kể từ lúc tạo lời nhắn)</li>
      </ul>

      <p style="font-size: 12px; color: #777; margin-bottom: 14px;">
        ⚠️  Xin lỗi vì sự bất tiện không thể tải ảnh trực tiếp về máy do giới hạn bảo mật của Zalo/Facebook. Vui lòng làm theo cách thủ công ở trên.
      </p>
